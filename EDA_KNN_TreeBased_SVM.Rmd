...
###
knn_cf<-table(knn.pred, test_set$stroke)
knn_cf
```
...
knn_df <- data.frame(
  Actual = c(0, 0, 1, 1),
  Predicted = c(0, 1, 0, 1),
  Freq = c(996, 218, 43, 21)
)

# Plot the confusion matrix using ggplot2 with adjusted font size
ggplot(data = knn_df, aes(x = Actual, y = Predicted, fill = Freq)) +
...
library(MASS)
library(tree)
tree.model = tree(stroke ~ ., data = balanced_train_set)
...
predictions_dt <- predict(tree.model, newdata = test_set, type = "class")

```


```{r}
dt_cf<-confusionMatrix(predictions_dt, test_set$stroke)
dt_cf
...
set.seed(25)
cv.trans=cv.tree(tree.model)
plot(cv.trans$size, cv.trans$dev, type = 'b')

```

```{r}

index = which.min(cv.trans$dev)
cat(cv.trans$size[index])
...
prune.trans= prune.tree(tree.model,best=7)
plot(prune.trans)
text(prune.trans)
...
```{r}
predicted <- predict(prune.trans, newdata = test_set, type = 'class')
# Calculate accuracy
accuracy <- mean(predicted == test_set$stroke)
accuracy

conf_matrix <- table(predicted, test_set$stroke)
...
dt_cf<-confusionMatrix(predicted, test_set$stroke)
dt_cf
```
...
dt_df <- as.data.frame(dt_cf$table)

# Plot the confusion matrix using ggplot2
ggplot(data = dt_df, aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile(color = "white") +
...
results <- data.frame(mtry=integer(), accuracy=numeric(), row.names=NULL)
best_accuracy <- 0 # Variable to track the best accuracy
best_importance <- NULL # Variable to store importance for the best model

for (m in 1:10) {
set.seed(25)
rf_model <- randomForest(stroke ~ ., data=balanced_train_set, mtry=m, ntree=500,
importance=TRUE)
predictions <- predict(rf_model, newdata=test_set)
accuracy <- sum(predictions == test_set$stroke) / nrow(test_set)
results <- rbind(results, data.frame(mtry=m, accuracy=accuracy))
# Update best_importance if current model has better accuracy
if(accuracy > best_accuracy){
best_accuracy <- accuracy
best_importance <- importance(rf_model) # Get variable importance for the
...
if (!is.null(best_importance)) {
importance_scores <- best_importance[, 'MeanDecreaseAccuracy'] #%INCMSE
top_variables <- sort(importance_scores, decreasing = TRUE)
top_n_variables <- names(top_variables)[1:6]
print(top_n_variables)
} else {
print('Variable importance is NULL.')
}
...
rf_model <- randomForest(stroke ~ ., data=balanced_train_set, mtry=4, ntree=500,
importance=TRUE)
...
rf_cf<-confusionMatrix(predictions, test_set$stroke)
rf_cf
```
...
# Convert the confusion matrix to a data frame for plotting
rf_df <- as.data.frame(rf_cf$table)

# Plot the confusion matrix using ggplot2
ggplot(data = rf_df, aes(x = Reference, y = Prediction, fill = Freq)) +
  geom_tile(color = "white") +
  geom_text(aes(label = sprintf("%0.2f", Freq)), vjust = 1) +
  scale_fill_gradient(low = "white" ,high = "lightblue") +
  labs(x = "Actual", y = "Predicted", title = "Confusion Matrix") +
